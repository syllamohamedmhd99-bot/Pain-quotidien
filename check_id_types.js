...
async function checkTypes() {
    ...
    const { data: delData } = await supabase.from('deliveries').select('id').limit(1);
    const { data: prodData } = await supabase.from('products').select('id').limit(1);

    console.log("Delivery ID sample:", delData?.[0]?.id, "Type:", typeof delData?.[0]?.id);
    console.log("Product ID sample:", prodData?.[0]?.id, "Type:", typeof prodData?.[0]?.id);
}
